...
这个方案最干净，但要多写一点 Editor 创建代码。
## OnBeforeSerialize序列化回调

`OnBeforeSerialize` 适合干一类事：把已有内存数据整理成 Unity 能保存的字段。
...
比如 Unity 不能直接序列化 `Dictionary`，就保存成两个 `List`：
...
这个可以，但也要小心：如果 ID 影响资产引用关系，最好别放这里。

适合做：

3. 同步缓存字段

比如运行时维护一个复杂结构，保存前转成简单数组：
...
public void OnBeforeSerialize()
{
    ...
    {
        _serializedItems.Add(item.ToSaveData());
    }
}
```
...
Unity 中的 `.asset`、`.scene`、`.prefab` 等都是 YAML 格式的文件。

在这个项目里，`DialogueGraph` 主资源会引用多个节点子资源。

### 对象块

Unity 的 asset 资源分为多个资源块。
