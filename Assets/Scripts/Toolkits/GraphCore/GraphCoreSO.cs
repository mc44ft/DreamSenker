...
{
    public abstract class GraphCoreSO : ScriptableObject
    {
        //给表现层返回具体的数据基类
        //给业务层返回泛型
    ...
    public class GraphCoreSO<TNode, TEdge> : GraphCoreSO, ISerializationCallbackReceiver
        ...
        where TEdge : GraphCoreEdge, new()
    {
        ...
        public TEdge GetEdgeByPortID(string portID)
        {
            ...
            if (_edgeLookup.ContainsKey(portID))
                return _edgeLookup[portID];
            return null;
        }
        private void RebuildLookups()
        {
            ...
            foreach (var edge in _edges)
            {
                ...
                _portToEdgeLookup[edge.ConnectionNodeID] = edge;
            }
